...
title: 로컬라이제이션
description: DHTMLX JavaScript RichText 라이브러리 문서에서 로컬라이제이션에 대해 알아볼 수 있습니다. 개발자 가이드와 API 레퍼런스를 살펴보고, 코드 예제와 라이브 데모를 직접 실행해 보세요. DHTMLX RichText 무료 30일 평가판을 다운로드할 수도 있습니다.
---

# 로컬라이제이션

RichText는 모든 인터페이스 레이블의 로컬라이제이션을 지원합니다. 새 로케일을 생성하거나 기본 제공 로케일을 수정한 후, 에디터 구성을 통해 적용할 수 있습니다.

## 기본 로케일

RichText는 기본적으로 영어 로케일을 적용합니다:
...
:::info
RichText에는 독일어(`de`)와 중국어(`cn`) 로케일도 기본 제공됩니다. `richtext.locales` 네임스페이스를 통해 기본 제공 로케일에 접근할 수 있습니다: `richtext.locales.en`, `richtext.locales.de`, `richtext.locales.cn`.
:::
...
모든 인터페이스 레이블에 대한 번역이 담긴 로케일 객체를 생성하거나 기본 제공 로케일을 수정합니다. 초기화 시 로케일을 적용하거나 아래와 같이 런타임에서 전환할 수 있습니다.

### 초기화 시 로케일 적용

생성자 구성의 [`locale`](api/config/locale.md) 속성에 로케일을 전달합니다:
...
초기화 이후 로케일을 변경하려면 [`setLocale()`](api/methods/set-locale.md) 메서드를 호출합니다:

~~~jsx
editor.setLocale(richtext.locales.cn);
~~~

기본 영어 로케일로 복원하려면 인수 없이 또는 `null`을 전달하여 [`setLocale()`](api/methods/set-locale.md)을 호출합니다:
