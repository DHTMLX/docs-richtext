...
~~~jsx {}
defaultStyles?: {
    "*"?: { // влияет на все блоки, позволяя задавать общие свойства для всех из них
...
В этом примере всем блокам `h2` назначается шрифт `"Roboto"` с размером 28px, а также изменяются цвет текста и цвет фона. CSS-стили применяются к блокам `h2` отдельно.
:::
