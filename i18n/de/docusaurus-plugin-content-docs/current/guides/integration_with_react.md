...
Legen Sie ein Container-Element für RichText fest und initialisieren Sie die Komponente mit dem `Richtext`-Konstruktor innerhalb von `useEffect()`. Rufen Sie die [`destructor()`](api/methods/destructor.md)-Methode in der Cleanup-Funktion auf, um RichText zu entfernen:
...
Alternativ können Sie die [`setValue()`](api/methods/set-value.md)-Methode innerhalb von `useEffect()` aufrufen, um Daten in RichText zu laden:
...
RichText löst Events bei Benutzeraktionen aus. Abonnieren Sie Events mit der [`api.on()`](api/internal/on.md)-Methode, um auf Benutzereingaben zu reagieren. Siehe die [vollständige Event-Liste](api/overview/events_overview.md).
