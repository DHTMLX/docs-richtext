...
title: Lokalisierung
description: In der Dokumentation der DHTMLX JavaScript RichText-Bibliothek erfahren Sie mehr über die Lokalisierung. Entwicklerhandbücher und API-Referenz, Code-Beispiele und Live-Demos sowie eine kostenlose 30-Tage-Evaluierungsversion von DHTMLX RichText.
---

# Lokalisierung

RichText unterstützt die Lokalisierung aller Oberflächenbeschriftungen. Erstellen Sie eine neue Locale oder passen Sie eine integrierte an, und wenden Sie das Ergebnis über die Editor-Konfiguration an.

## Standard-Locale

Standardmäßig verwendet RichText die englische Locale:
...
:::info
RichText enthält außerdem die integrierten Locales für Deutsch (`de`) und Chinesisch (`cn`). Auf alle integrierten Locales kann über den Namespace `richtext.locales` zugegriffen werden: `richtext.locales.en`, `richtext.locales.de`, `richtext.locales.cn`.
:::
...
## Benutzerdefinierte Locale anwenden

Erstellen Sie ein Locale-Objekt (oder passen Sie eine integrierte Locale an) mit Übersetzungen für alle Oberflächenbeschriftungen. Wenden Sie die Locale bei der Initialisierung an oder wechseln Sie sie zur Laufzeit, wie unten gezeigt.

### Locale bei der Initialisierung anwenden

Übergeben Sie die Locale an die Eigenschaft [`locale`](api/config/locale.md) in der Konstruktorkonfiguration:
...
Rufen Sie die Methode [`setLocale()`](api/methods/set-locale.md) auf, um die Locale nach der Initialisierung zu ändern:

~~~jsx
editor.setLocale(richtext.locales.cn);
~~~

Um die englische Standard-Locale wiederherzustellen, rufen Sie [`setLocale()`](api/methods/set-locale.md) ohne Argumente oder mit `null` auf:
