...
sidebar_label: 本地化
title: 本地化
description: 您可以在 DHTMLX JavaScript RichText 库的文档中了解本地化内容。浏览开发者指南和 API 参考，尝试代码示例和在线演示，并下载 DHTMLX RichText 的免费 30 天评估版本。
---

# 本地化

JavaScript RichText 界面中的所有标签都可以进行本地化。为此，您可以创建一个新的语言环境或调整现有语言环境，然后将其设置到 RichText 上。
...
:::info
除了默认的 ***en*** 语言环境（*英语*），RichText 还内置了 ***de***（*德语*）和 ***cn***（*中文*）语言环境。
:::
...
- 创建自定义语言环境（或更新默认语言环境），并为所需的所有标签添加翻译（可以是任何语言）

- 通过 [`locale`](api/config/locale.md) 属性或 [`setLocale()`](api/methods/set-locale.md) 方法为 **RichText** 设置该语言环境
