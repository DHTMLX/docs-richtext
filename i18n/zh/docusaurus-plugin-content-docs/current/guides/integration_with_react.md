...
title: 与 React 集成
description: 您可以在 DHTMLX JavaScript RichText 库的文档中了解与 React 的集成方式。浏览开发者指南和 API 参考，查看代码示例和在线演示，并下载 DHTMLX RichText 的 30 天免费试用版本。
---
...
创建一个 React 组件，将 RichText 添加到应用中。在 *src/* 目录下，创建一个名为 *Richtext.jsx* 的新文件。
...
为 RichText 设置容器元素，并在 `useEffect()` 中使用 `Richtext` 构造函数初始化组件。在清理函数中调用 [`destructor()`](api/methods/destructor.md) 方法以移除 RichText：
...
打开 *App.js* 并导入数据。将值以 prop 的形式传递给 `<RichText/>` 组件：
...
或者，在 `useEffect()` 中调用 [`setValue()`](api/methods/set-value.md) 方法将数据加载到 RichText 中：
...
RichText 在用户操作时会触发事件。使用 [`api.on()`](api/internal/on.md) 方法订阅事件以响应用户输入。请参阅[完整事件列表](api/overview/events_overview.md)。

打开 *Richtext.jsx* 并更新 `useEffect()` hook。以下示例在每次 [`print`](api/events/print.md) 事件触发时记录一条消息：
...
您现在已在 React 中完成了 RichText 的集成。根据实际需求自定义代码。完整示例可在 [GitHub](https://github.com/DHTMLX/react-richtext-demo) 上获取。
